...
ALL issue-worthy findings (concurrency, over-threshold flakes/slow-failing
workflows/burners) land as `ci-audit` issues in a CENTRAL GitHub tracker — the SRE
function-brain, the infra/CI-health brain that already owns health sweeps + weekly
retros. NOT the audited source repos, and NOT Linear (retired). One `ci-audit`
issue per finding. (`.github` was the first pick but has Issues disabled — the SRE
brain is the home.) The tracker repo + the audited-repo owner are read from the
AUDIT_TRACKER_REPO / AUDIT_ORG env (the workflow sets them per org).

These issues are the INPUT to the actuator — the SRE brain's ci-fix-sweep + ci-fix-
...
minted release-bot token, which has issues:write org-wide. Diagnostics +
...
# Central tracker = the SRE function-brain (infra/CI-health). Per-repo routing is
# the alternative: file in the audited repo (the release-bot has org-wide
...
# e.g. "luminality-web"). The engine needs a fully-qualified owner/name, so we
...
def strip_owner(repo: str) -> str:
    """`luminalityai/luminality-web` -> `luminality-web` (match audit's stripped names)."""
    return repo.split("/")[-1]
