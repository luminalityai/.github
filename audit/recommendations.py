...
the org's CI-health tracker. It opens NO PRs — the fix is the actuator's job (the
SRE brain's ci-fix-sweep + ci-fix-engine), which reads these issues, proposes a
guarded workflow fix, and opens a ready-for-review PR that links back to the
originating issue. Keeping the sensor PR-free is the whole point: one auditable
work unit per finding, fixed (or not) downstream where a human gates it.
...
Org-shared workflows (e.g. the `Claude Code Review` reusable defined in this org's
`.github` repo and called by every repo) are COALESCED in the digest: one systemic
root cause that used to fan out into one issue per caller repo (e.g. 10 identical
Claude Code Review issues) is now a SINGLE digest line listing the affected repos.
Coalescing applies to the digest only — issue-worthy findings stay per-repo because
the ci-fix engine fixes exactly one workflow file per issue.
...
The workflow's scope INCLUDES this org's own `.github` repo (locked decision), so
its reusable workflows are audited like any other.
"""
...
# Org-shared workflows defined as reusables in this org's `.github` repo and rolled
...
def partition(data: dict) -> dict:
    ...
    for w in workflows:
        ...
        # The ci-fix engine reads the issue, re-verifies the block is genuinely
        # missing, and opens a guarded ready-for-review PR. We still run the cheap
        # pre-check here so already-fixed workflows never become noise.
