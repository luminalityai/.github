...
def test_split_thresholds():
    ...
    data = {"generated_at": "t", "window_days": 7, "scope": "all", "workflows": [
        ...
        wf("luminality-web", "CI", runs=40, failure=25, failed_minutes=120, failure_rate=0.62, total_minutes=50),
        # digest: failed-minutes below bars
        wf("luminality-app", "CI", runs=30, failure=4, failed_minutes=12, failure_rate=0.13, total_minutes=40),
        # issue-worthy: concurrency (always)
        wf("luminality-sre", "Deploy", cancelled=6, cancel_rate=0.30, cancelled_minutes=40, total_minutes=30),
        # issue-worthy: flakes >= 3
        wf("delivery-ops", "Nightly", flake_count=5, total_minutes=20),
    ]}
    out = rec.partition(data)
    j = {f"{x['repo']}/{x['category']}" for x in out["judgment"]}
    assert j == {"luminality-web/failed-minutes", "luminality-sre/concurrency",
                 "delivery-ops/flakes"}, j
    d = {f"{x['repo']}/{x['category']}" for x in out["digest"]}
    assert "luminality-app/failed-minutes" in d, d
    assert out["scanned_repos"] == sorted(
        {"luminality-web", "luminality-app", "luminality-sre", "delivery-ops"}), out["scanned_repos"]
...
def test_coalesce_shared():
    rec.gh_raw_workflow = lambda r, p: None
    data = {"generated_at": "t", "window_days": 7, "scope": "all", "workflows": [
        wf("luminality-web", "Claude Code Review", flake_count=1),
        wf("luminality-app", "Claude Code Review", flake_count=2),
        wf("delivery-ops", "Claude Code Review", flake_count=1),
    ]}
    out = rec.partition(data)
    flake_digest = [x for x in out["digest"] if x["category"] == "flakes"]
    assert len(flake_digest) == 1, flake_digest          # 3 repos -> 1 coalesced row
    assert flake_digest[0]["coalesced_repos"] == ["delivery-ops", "luminality-app", "luminality-web"]
...
def test_reconcile_rails():
    ...
    oi.list_open_ci_audit_issues = lambda: [
        {"number": 101, "body": "<!-- ci-target repo=luminalityai/luminality-web workflow=x category=flakes -->\n<!-- actions-audit:a1a1a1a1a1a1 -->"},
        {"number": 102, "body": "<!-- ci-target repo=luminalityai/luminality-web workflow=x category=concurrency -->\n<!-- actions-audit:b2b2b2b2b2b2 -->"},
        {"number": 103, "body": "human-filed, no marker"},
        {"number": 104, "body": "<!-- ci-target repo=luminalityai/unscanned-repo workflow=x category=flakes -->\n<!-- actions-audit:c3c3c3c3c3c3 -->"},
    ]
    n = oi.reconcile({"b2b2b2b2b2b2"}, {"luminality-web", "luminality-app"}, "http://run/1")
